"""Functions to load data from CSVs generated by crawl.py"""

from collections import defaultdict
from crawl import User, Page
...
class Loader:
    """Loads and holds user and page data
    from CSV files generated by Crawler."""
    ...
    def load_userdefs(self, open_file):
        """Load user definitions from open CSV file"""
    ...
    def process_user_month_line(self, lineobj):
        """Given a UserPageMonthLine object,
        process it for user data."""
        if lineobj.user_id is None:
            return
        user = self.users[lineobj.user_id]
        user.is_bot = bool(int(lineobj.is_bot))
        user.all_namespace_page_ids.add(lineobj.page_id)
        edits = int(lineobj.page_month_edits)
        user.all_namespace_months[lineobj.month] += edits
        if lineobj.namespace == "0":
            user.months[lineobj.month] += edits
            user.page_ids.add(lineobj.page_id)
        elif lineobj.namespace in ["4", "5"]:
            user.project_namespace_months[lineobj.month] += edits

    def load_users(self, months_filepath="user_page_months_output.csv",
                   users_filepath="users_output.csv",
                   limit=None):
        """Load dict of users from CSV files in format:
        {(user_id, User)}."""
        with open(users_filepath, encoding="utf-8") as userdefs:
            self.load_userdefs(userdefs)
        message = "{} user defs loaded".format(len(self.users))
        print(message)
        with open(months_filepath, encoding="utf-8") as lines:
            linecount = 0
            for line in lines:
                linecount += 1
                ...
                if linecount == 1 and lineobj.user_id is None:
                    # skip header
                    continue
                self.process_user_month_line(lineobj)
                self.stats.process_line(lineobj)
...
class BasicStats:
    ...
    def process_line(self, lineobj):
        ...
        if lineobj.namespace == "0":
            self.mainspace_page_ids.add(lineobj.page_id)
...
def is_deletion_page(page_name):
    if "deletion/" in page_name:
        return True
    elif "Deletion/" in page_name:
        return True
    else:
        return False
